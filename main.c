...
void print_array(int* array, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        // Dodaj przecinek poza ostatnim elementem
        if (i < size - 1) {
            printf("\n ");
        }
    }
...
int meta_local_search_tsp(int* path, int fuel) {
    ...
    int length = calculate_path_length_tsp(current_path, fuel);

    for (int i = 0; i < IT; i++) {
        change_path(current_path, alternate_path, N + 1);
        if (check_if_route_ok(alternate_path, gas_stations, fuel)) {
            int alternate_length = calculate_path_length_tsp(alternate_path, fuel);
            if (alternate_length < length) {
                length = alternate_length;
                copy_array(alternate_path, current_path, N + 1);
            }
    ...
    if (!check_if_route_ok(current_path, gas_stations, fuel)) {
        return 0;
    }
